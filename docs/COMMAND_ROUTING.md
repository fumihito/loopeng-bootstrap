...
| planning | `frame-decision-making` | Commitment decisions with dry facts, modal scoring, and revisit tripwires | Do you need to make or revisit a commitment under real constraints, with explicit will / can / must scoring? | If the commitment is already made and the work needs phasing, hand off to `frame-plandev`; if you need dependency structure instead of a commitment, hand off to `frame-plantask`; if the input is already a note or plan to compress, hand off to `frame-smeac`; if the question is architecture selection before commitment, hand off to `frame-research-arch`. |
| planning | `frame-plandev` | Multi-step delivery planning after a commitment is chosen | Do you need a phased delivery plan that includes decisions, verification, and the next handoff? | If the main work is deciding whether to commit, hand off to `frame-decision-making`; if the main work is dependency structure, hand off to `frame-plantask`; if you are compressing existing notes into a brief, hand off to `frame-smeac`. |
...
| planning | `frame-smeac` | Brief compression from existing notes | Is the input already a plan, incident note, or discussion that needs to be compressed into a handoffable brief? | If a new phased plan is needed, hand off to `frame-plandev`; if the structure still needs explicit dependencies, hand off to `frame-plantask`; if the real question is whether to commit, hand off to `frame-decision-making`. |
...
| thinking-check | `frame-wall` | Premise challenge and reframing of the user's own problem definition | Is the issue the answer, or the frame that is producing the answer? | If the problem has become a testable claim, hand off to `frame-critical-review`; if the problem is hidden assumptions or avoided alternatives, hand off to `frame-blind-spot`; if the judgment is inherited or habitual, hand off to `frame-inertia`; if the request still needs decomposition, hand off to `frame-first-principles`. |
...
The planning cluster is intentionally split by question:

- `frame-decision-making` names the commitment and the tripwires that would revisit it.
- `frame-plandev` phases an already chosen commitment.
- `frame-plantask` exposes ordering and dependency structure.
- `frame-smeac` compresses existing notes into a brief.
