...
Tool-trace Markdown views under `.agent-loop/state/reports/trace/` are deterministic observational artifacts generated by `python3 -m loopeng trace render`; they do not have the governance status of the Run Report. See [`docs/TOOL_TRACE.md`](TOOL_TRACE.md) for the schema and operating rules.
