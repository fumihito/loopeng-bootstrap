...
## OKF LLMWiki installation discipline

The target may already contain `llmwiki/`. Preserve it as project-owned durable memory.

1. Do not replace the directory or existing concepts with the package skeleton.
2. Allow the deterministic installer to create only missing skeleton files and directories.
3. Do not rewrite existing OKF frontmatter or body formatting merely for style.
4. If an existing bundle is invalid, report the exact validator findings and resolve them by semantic review; never delete the bundle to make validation pass.
5. Do not import configuration contents, prompts, transcripts, tool outputs, command arguments, or secrets into LLMWiki.
6. Confirm that `.agent-loop/bin/okfctl`, its Go source, memory policy, Memory Curator role, and OKF documentation were installed.
7. Run `.agent-loop/bin/okfctl validate --root llmwiki` before declaring installation complete.
