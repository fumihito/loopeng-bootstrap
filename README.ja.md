...
`loop_status.py` は、決定論的な runtime 成果物だけを使って現在のループ状態を描画します。
...
HTML は既定で `.agent-loop/runtime/status.html` に書き出され、オフラインでそのまま読めます。既定表示では Loop Brief の goal 文は出しません。ローカルページで brief 文を見たいときだけ `--include-brief` を使ってください。詳細は `docs/OBSERVABILITY.md` を参照してください。
`tests/test_loop_status.py::LoopStatusTests` で検証しています。
