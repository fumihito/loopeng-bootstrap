...
description: Diagnose a tripped or repeating loop and produce a bounded recovery, rollback, or escalation plan.
---

Invoke the platform custom subagent named `watchdog-recovery` in an isolated read-only context. Explicitly spawn exactly one `watchdog-recovery` subagent, give it the current task and relevant loop state, wait for it to finish, and return its result to the parent flow.
