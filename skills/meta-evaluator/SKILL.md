...
Invoke the platform custom subagent named `meta-evaluator` in an isolated context. Explicitly spawn exactly one `meta-evaluator` subagent, give it the current task and relevant loop state, wait for it to finish, and return its result to the parent flow.
