...
class InboxModelTests(unittest.TestCase):
    ...
    def test_action_ctrl_c_rejects_input_and_exit_requires_confirmation(self) -> None:
        ...
        screen.getch.return_value = ord("n")
        screen.getch.side_effect = None
        with mock.patch("loopeng.inbox_tui.curses.flushinp"), mock.patch("loopeng.inbox_tui.curses.noecho"):
    ...
    def test_main_screen_ctrl_c_uses_exit_confirmation(self) -> None:
        ...
        try:
            screen = mock.Mock()
            screen.getmaxyx.return_value = (24, 80)
            screen.getch.side_effect = [KeyboardInterrupt, ord("n"), ord("q"), ord("y")]
